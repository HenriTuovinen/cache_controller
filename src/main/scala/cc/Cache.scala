...
class Cache(index_len: Int, tag_len: Int, data_len: Int, debug: Boolean = false) extends  Module { 
  ...
  // maybe scala if statement so this is done only once (either done or not)
  if(debug){
